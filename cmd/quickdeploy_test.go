...
func TestWriteDeployConf_dnspodMultiLine(t *testing.T) {
	...
	// key 为完整子域名：ConfFileName 与 scanDNSPodConfDir 均按 sub_domain.domain 生成
	//（避免同根域不同子域互相覆盖），故 www + example.com → www.example.com。
	d, ok := cfg.DNSPodDomains["www.example.com"]
	if !ok {
...
func TestQuickDeployCore_orchestration(t *testing.T) {
	...
	syncRunner = func(ctx context.Context, dir string, _ string, _ ...string) (*sync.SyncSummary, error) {
		return &sync.SyncSummary{Updated: 3}, nil
	}
	scheduled := false
