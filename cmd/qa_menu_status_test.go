...
func TestBuildStatusLine_allOk(t *testing.T) {
	...
	// 1) CF DNS: create cf-dns dir with .conf
	...
	// 2) DNSPod: create dnspod dir with .conf
...
// TestBuildStatusLine_threeModules status line contains all 3 module indicators.
func TestBuildStatusLine_threeModules(t *testing.T) {
	tmp := t.TempDir()
	origCfgDir := cfgDir
	cfgDir = tmp
	defer func() { cfgDir = origCfgDir }()
	...
	line := buildStatusLine()
	modules := []string{"CF DNS", "DNSPod", "cfst"}
