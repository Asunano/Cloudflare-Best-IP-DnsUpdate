...
func (s *speedtestService) Run(ctx context.Context, onProgress speedtest.ProgressFunc) ([]speedtest.SpeedResult, error) {
	...
	results, runErr := tester.Run(ctx, "./assets/data/cf-ip", onProgress)
...
func (s *daemonService) build() (*scheduler.Daemon, error) {
	...
	// 传入配置目录绝对路径：daemon.install（IPC）安装的服务单元同样需要正确的
	// WorkingDirectory（配置目录父目录）与 --config-dir 参数（与 CLI `schedule install` 一致）。
	absDir, _ := filepath.Abs(s.cfgDir)
	return scheduler.NewDaemonWithConfigDir(sched, cfg, parseInterval(cfg), false, absDir), nil
}
