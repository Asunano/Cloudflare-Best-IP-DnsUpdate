...
// 不做任何删除/创建（就地更新，无 churn）；其余情况一律集合法 diff
// （删多余 旧∩¬目标 + 建缺失 目标∩¬旧），绝不索引复用已删除记录。
func applyLineDiff(ctx context.Context, prov LineAwareProvider, domain, sub, line string, existing []Record, targetIPs []string, ttl int, allowDelete bool, res *SyncResult) {
	...
	// 删除多余（受 allowDelete 限制；不允许删除时保留旧记录以固化）。
	for content, id := range existingSet {
		if !targetSet[content] {
			if !allowDelete {
				continue
			}
			if err := prov.DeleteLineRecord(ctx, domain, id); err != nil {
				res.Errors = append(res.Errors, sub+": "+err.Error())
				continue
			}
			res.Deleted++
	...
	for _, ip := range targetIPs {
		if _, ok := existingSet[ip]; !ok {
			if err := prov.UpsertLineRecord(ctx, domain, sub, line, ip, ttl); err != nil {
				res.Errors = append(res.Errors, sub+": "+err.Error())
				continue
			}
			res.Created++
