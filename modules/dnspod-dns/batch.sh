#!/bin/bash
# ==============================================================================
# cfopt - DNSPod DNS 批量更新器 (Batch Updater)
# Version: 0.1
# Description: 遍历 conf/dnspod/ 目录中的所有域名配置，依次执行 DNS 更新
# Usage: bash modules/dnspod-dns/batch.sh
# ==============================================================================
SCRIPT_VERSION="0.1"
...
DEFAULT_CONFIG="${ROOT_DIR}/conf/dnspod.json"
...
# 2. 如果没有找到新格式的配置，检查旧的默认配置
if [[ ${#CONFIG_FILES[@]} -eq 0 ]] && [[ -f "$DEFAULT_CONFIG" ]]; then
    echo -e "${YELLOW}[INFO] 未找到多域名配置，使用默认配置文件${NC}"
    CONFIG_FILES+=("$DEFAULT_CONFIG")
fi

# 3. 如果仍然没有配置，退出
if [[ ${#CONFIG_FILES[@]} -eq 0 ]]; then
    echo -e "${RED}[ERROR] 未找到任何 DNSPod DNS 配置文件${NC}"
