...
if [[ ${#CONFIG_FILES[@]} -eq 0 ]] && [[ -f "${ROOT_DIR}/conf/cf-dns.json" ]]; then
    ...
    if command -v jq &>/dev/null; then
        domain_name=$(jq -r '.dns.domain // "unknown"' "${ROOT_DIR}/conf/cf-dns.json" 2>/dev/null)
    fi
    ...
    # 复制并重命名配置文件
    cp "${ROOT_DIR}/conf/cf-dns.json" "${CONFIG_DIR}/${domain_name}.json"
    CONFIG_FILES=("${CONFIG_DIR}/${domain_name}.json")
    
    echo -e "${GREEN}[OK] 已迁移到多域名格式: ${domain_name}.json${NC}"
    echo -e "${GRAY}提示: 建议删除旧的 conf/cf-dns.json 文件${NC}"
    echo ""
fi
