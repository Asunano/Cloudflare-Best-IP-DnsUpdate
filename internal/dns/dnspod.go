	...
	for attempt := 0; attempt < p.maxRetry; attempt++ {
		...
		if resp.Response.Error.Code != "" {
			...
			if strings.HasPrefix(code, "LimitExceeded") {
				return nil, common.New("dnspod:limit", code+": "+resp.Response.Error.Message)
			}
			last = &resp
			continue
		}
...
func (p *DNSPodProvider) modifyRecord(ctx context.Context, domain, subDomain, line, value string, ttl int, recordID int64) error {
	payload := mustJSON(map[string]any{
		"Domain":     domain,
		"SubDomain":  subDomain,
		"RecordType": RecordTypeA,
...
func (p *DNSPodProvider) deleteRecord(ctx context.Context, domain string, recordID int64) error {
	payload := mustJSON(map[string]any{
		"Domain":   domain,
		"RecordId": recordID,
	})
	...
	for _, r := range existing {
		if r.Content == value {
			...
			if r.TTL != effTTL {
				...
				return common.Wrap("dnspod:upsert:modify", p.modifyRecord(ctx, domain, subDomain, line, value, effTTL, id))
			}
			return nil
		}
	}
...
func (p *DNSPodProvider) DeleteLineRecord(ctx context.Context, domain, recordID string) error {
	...
	return p.deleteRecord(ctx, domain, id)
}

// UpdateLineRecord 直接更新指定 ID 的记录为新的 IP 值（不额外 List，减少 API 调用）。
func (p *DNSPodProvider) UpdateLineRecord(ctx context.Context, domain, recordID, subDomain, line, value string, ttl int) error {
	id, _ := strconv.ParseInt(recordID, 10, 64)
	return common.Wrap("dnspod:update", p.modifyRecord(ctx, domain, subDomain, line, value, ttl, id))
}
...
func (p *DNSPodProvider) UpsertRecord(ctx context.Context, _ string, rec Record) error {
	...
	if rec.ID != "" {
		...
		return p.modifyRecord(ctx, p.domain, sub, line, rec.Content, rec.TTL, id)
	}
...
func (p *DNSPodProvider) DeleteRecord(ctx context.Context, _ string, id string) error {
	...
	return p.deleteRecord(ctx, p.domain, pid)
}
