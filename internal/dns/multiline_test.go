...
func (m *memLineProvider) UpdateLineRecord(_ context.Context, _ string, recordID, _, _, value string, _ int) error {
	for k, recs := range m.store {
		for i, r := range recs {
			if r.ID == recordID {
				m.store[k][i].Content = value
				return nil
			}
		}
	}
...
func TestSyncMultiLine_DeleteExcessAndCreateMissing(t *testing.T) {
	...
	// 预置 3 条旧记录：1.1.1.1（保留）、2.2.2.2（更新→8.8.8.8）、3.3.3.3（删除）。
	k := memKey("www", "默认")
	m.store[k] = []Record{
	...
	require.NotNil(t, res)
	assert.Equal(t, 0, res.Created, "复用 old2 更新为 8.8.8.8，无需新增")
	assert.Equal(t, 1, res.Deleted, "应删除 3.3.3.3")
	assert.Equal(t, 1, res.Updated, "old2 2.2.2.2 应更新为 8.8.8.8")
...
func TestSyncMultiLine_DeleteModeNoneKeepsExcess(t *testing.T) {
	...
	k := memKey("www", "默认")
	m.store[k] = []Record{{ID: "old1", Content: "9.9.9.9", TTL: 600}} // 多余但 DeleteMode=none → 保留不删
	...
	assert.Equal(t, 0, res.Created, "智能更新复用 old1 更新为 1.1.1.1，无需新增")
	assert.Equal(t, 1, res.Updated, "old1 9.9.9.9 应更新为 1.1.1.1")

	recs, _ := m.ListLineRecords(context.Background(), "example.com", "www", "默认")
	assert.Len(t, recs, 1, "none 模式不删但智能更新复用现有记录槽位")
	assert.Equal(t, "1.1.1.1", recs[0].Content, "旧记录内容应被更新为新 IP")
}
