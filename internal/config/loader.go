...
// Load 从 dir 目录加载 global.json / cf-dns.json / dnspod.json 三个配置文件，
// 使用 sync.Once 缓存首次结果。
// cf-ip.json 已废弃，不再读取。
...
func loadDir(dir string) (*Config, error) {
	cfg := &Config{}
	var err error
	// global.json 必需（含 lock_dir、log_dir、调度间隔等全局设置）。
	...
	// cf-dns.json、dnspod.json 为可选顶层配置，不存在时留 nil，模块靠域名级配置 (*.conf) 运行。
	cfg.CFDNS, _ = readJSON[CFDNSConfig](filepath.Join(dir, "cf-dns.json"))
	cfg.DNSPod, _ = readJSON[DNSPodConfig](filepath.Join(dir, "dnspod.json"))
...
// key 优先取文件内 sub_domain.domain（完整子域名）；无 sub_domain 或空时回退 domain 字段；
// 均空时以文件名（去扩展名）为准。与 Bash 多域名 *.json 配置向后兼容。
func scanDNSPodConfDir(dir string) map[string]*DNSPodConfig {
	...
	for stem, data := range scanDomainConfBytes(dir, "dnspod") {
		...
		if strings.TrimSpace(v.Domain) != "" {
			key = v.Domain
			sub := strings.TrimSpace(v.SubDomain)
			if sub != "" && sub != "@" {
				key = sub + "." + key
			}
		}
...
func applyDefaults(cfg *Config) {
	...
	if cfg.Global.SpeedTest.MaxRetry == 0 {
		cfg.Global.SpeedTest.MaxRetry = 3
	}
}
...
// Save 将配置写回 dir 目录的三个 JSON 文件（供 GUI/热加载保存设置）。
