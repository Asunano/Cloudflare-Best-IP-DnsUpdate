...
func (fakeTester) Run(ctx context.Context, outputDir string, progress speedtest.ProgressFunc) ([]speedtest.SpeedResult, error) {
...
func TestSyncAll_PhaseCount(t *testing.T) {
	...
	assert.Equal(t, 2, lastTotal, "总数应为 2 个启用模块（无全局测速阶段）")
	assert.Equal(t, 2, lastCur)
}

// TestSyncAll_ProvidersWithError 验证某模块 Sync 报错时不计入返回错误，而是收集到 summary.Errors。
func TestSyncAll_ProvidersWithError(t *testing.T) {
	...
	syn := NewSyncer(fakeTester{}, reg, &fakeHistory{})
	summary, err := syn.SyncAll(context.Background(), &config.Config{}, nil)
	require.NoError(t, err, "模块局部失败不应导致 SyncAll 整体返回错误")
	require.Len(t, summary.Errors, 1, "失败信息应收集到 summary.Errors")
	assert.Contains(t, summary.Errors[0], "boom")
}
